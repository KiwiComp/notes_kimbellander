...
const deleteNote = async (event) => {
    ...
    try {
        ...
        console.error(err);
        return sendResponse(500, {message: "Could not delete the requested note.", error: err.message});
    };

    try {
        const deletedNote = await storeNoteWithNewPrefixSK(noteToDelete, DELETED_NOTES_PREFIX);
        return sendResponse(200, {message: "Note has been successfully deleted and stored as deleted.", deletedNote: deletedNote});
    } catch(err) {
        console.error(err);
        return sendResponse(500, {message: "Deleted note failed to be stored.", error: err.message});
    };
